...
res={'reference_40ns_frame_index':int(f40),'reference_40ns_time_ps':float(t40),'reference_40ns_pocket_atoms':int(pocket40.sum()),'reference_40ns_core_pocket_pairs':int(len(pair40)),'reference_40ns_pocket_residues':pocket40_keys,'minimized_reference_pocket_atoms':int(pocketmin.sum()),'minimized_reference_core_pocket_pairs':int(len(pairmin)),'overall_40_70ns':summ(a),'first_half_40_55ns':summ(a[a[:,0]<=55000.1]),'second_half_55_70ns':summ(a[a[:,0]>=55000.0])}
for name,col in [('core_rmsd_vs_40ns',1),('branch1_rmsd_vs_40ns',3),('branch2_rmsd_vs_40ns',4),('com_drift_vs_40ns',5),('pair_retention_vs_40ns',7),('nearest_distance_to_40ns_pocket',9)]:res['overall_40_70ns'][name+'_slope_per_ns']=float(np.polyfit(a[:,0]/1000,a[:,col],1)[0])
res['interpretation']='Using the 40 ns frame as a local reference removes the 0-10 ns relocation from the stability test. A small core RMSD/COM drift and stationary pocket nearest distance/contact fraction over 40-70 ns indicate no ongoing departure; branch RMSDs quantify local flexible motion.'
