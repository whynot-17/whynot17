...
# (a) ranked bubble landscape -> cross-type inherit -> param inherit
...
def build_audit_tables(source: Path, top30, detect, comparison, primary):
    ...
    top30_out.to_csv(source / "figure2_top30_screen_v2.csv", index=False)
    ...
    molecular_or = {
        "BBzP": "OR 5.95", "MBzP": "OR 10.18", "Nickel monoxide": "OR 6.03",
        "MEHP": "OR 4.34", "GenX": "OR 4.05", "Gallium arsenide": "OR 2.88",
        "TBPH": "OR 2.83", "MiNP/DINP": "OR 10.06",
    }
    collision = {
        "BBzP": "0", "MBzP": "0", "Nickel monoxide": "3", "MEHP": "4",
        "GenX": "0", "Gallium arsenide": "0", "TBPH": "Not audited", "MiNP/DINP": "0",
    }
    matrix = pd.DataFrame([
        ("BBzP", 1, molecular_or["BBzP"], "PASS", "MBzP proxy", "10", collision["BBzP"], "Proxy null", f"OR {mbzp.OR:.3f}; P {mbzp.P:.3f}", "No"),
        ("MBzP", 2, molecular_or["MBzP"], "PASS", "MBzP", "10", collision["MBzP"], "Null", f"OR {mbzp.OR:.3f}; P {mbzp.P:.3f}", "No"),
        ("Nickel monoxide", 3, molecular_or["Nickel monoxide"], "PASS", "Not frozen", "—", collision["Nickel monoxide"], "Not evaluated", "—", "No"),
        ("MEHP", 7, molecular_or["MEHP"], "PASS", "MEHP", "10", collision["MEHP"], "Null", f"OR {mehp.OR:.3f}; P {mehp.P:.3f}", "No"),
        ("GenX", 8, molecular_or["GenX"], "PASS", "Not frozen", "—", collision["GenX"], "Not evaluated", "—", "No"),
        ("Gallium arsenide", 18, molecular_or["Gallium arsenide"], "PASS", "Not frozen", "—", collision["Gallium arsenide"], "Not evaluated", "—", "No"),
        ("TBPH", 23, molecular_or["TBPH"], "PASS", "Not frozen", "—", collision["TBPH"], "Not evaluated", "—", "No"),
        ("MiNP/DINP", 24, molecular_or["MiNP/DINP"], "PASS", "MCOP", "7", collision["MiNP/DINP"], "Positive", f"OR {mcop.OR:.3f}; P {mcop.P:.4f}", "ADVANCE"),
    ], columns=["candidate", "screen_rank", "molecular_signal", "molecular_stable",
                "human_biomarker", "nhanes_cycles", "pubmed_crc_hits",
                "human_crc_gate", "human_crc_result", "decision"])
    matrix.to_csv(source / "figure2_triage_matrix_v2.csv", index=False)
    ...
    pd.DataFrame(collision_rows, columns=["candidate", "candidate_query", "crc_query", "hit_count", "audit_date", "scope_note"]).to_csv(
        source / "figure2_pubmed_collision_audit_v2.csv", index=False)

    detect[["analyte", "n_cycles", "analytic_value_n", "above_lod_n", "above_lod_pct"]].to_csv(
        source / "figure2_biomarker_translation_v2.csv", index=False)
    return top30_out, matrix, mcop


def draw_panel_a(ax, top30):
    panel_title(ax, "A", "Top-30 molecular landscape")
    plot = top30.sort_values("screen_rank").copy()
    x = plot["screen_rank"].to_numpy(float)
    y = plot["minus_log10_bh_fdr"].to_numpy(float)
    overlap = plot["crc_overlap"].to_numpy(float)
    class_key = plot["chemical_class"].astype(str).str.lower().str.replace("[", "", regex=False).str.replace("]", "", regex=False)
    colors = {
        "phthalates": BLUE_MID,
        "heavy_metals": NEUTRAL_DARK,
        "pahs": ORANGE,
        "pfas_perfluoro": "#7A5A9E",
        "vocs": "#5D8A8A",
        "environmental_pollutants": "#A68A5B",
    }
    markers = {
        "phthalates": "o",
        "heavy_metals": "^",
        ...
        "pfas_perfluoro": "s",
        "vocs": "P",
        "environmental_pollutants": "X",
    }
    sizes = 20 + 6.5 * np.sqrt(overlap)
    for chemical_class in class_key.unique():
        keep = class_key.eq(chemical_class).to_numpy()
        ax.scatter(x[keep], y[keep], s=sizes[keep], marker=markers.get(chemical_class, "o"),
                   color=colors.get(chemical_class, NEUTRAL_LIGHT), alpha=0.82,
                   edgecolor=WHITE, linewidth=0.45, zorder=2)
    minp = plot["short_name"].eq("MiNP").to_numpy()
    ax.scatter(x[minp], y[minp], s=sizes[minp] * 1.45, marker="o", color=BLUE,
               edgecolor="#0E4776", linewidth=1.25, zorder=4)
    ax.axhline(-np.log10(0.05), color=NEUTRAL, linewidth=0.65, linestyle=(0, (3, 2)), zorder=0)
    ax.text(30.7, -np.log10(0.05) + 0.08, "BH-FDR 0.05", ha="right", va="bottom",
            fontsize=5.1, color=NEUTRAL)
    ax.annotate("MiNP  rank 24\nOR 10.06; empirical FDR 0.036",
                xy=(24, float(plot.loc[plot["short_name"].eq("MiNP"), "minus_log10_bh_fdr"].iloc[0])),
                xytext=(18.0, 8.2), ha="left", va="top", fontsize=5.9, color=BLUE,
                fontweight="bold", linespacing=1.15,
                arrowprops=dict(arrowstyle="-", color=BLUE, linewidth=0.8),
                bbox=dict(boxstyle="round,pad=0.25", facecolor=BLUE_PALE, edgecolor=BLUE, linewidth=0.7))
    ax.set_xlabel("Molecular screen rank", labelpad=3)
    ax.set_ylabel(r"$-\log_{10}$(BH-FDR)", labelpad=3)
    ax.set_xlim(0, 31)
    ax.set_ylim(0, max(y) * 1.13)
    ax.set_xticks([1, 5, 10, 15, 20, 24, 30])
    ax.grid(axis="y", color="#E8ECEE", linewidth=0.45)
    ax.grid(axis="x", color="#F0F2F3", linewidth=0.4)
    ax.text(0.01, 0.99, "Point size ~ CRC overlap",
            transform=ax.transAxes, ha="left", va="top", fontsize=5.3, color=NEUTRAL)
    ax.text(0.01, -0.16,
            "Shapes: phthalates o  metals ^  PAHs <>  PFAS []  VOCs +",
            transform=ax.transAxes, ha="left", va="top", fontsize=5.0, color=NEUTRAL)


def draw_panel_b(ax, top30):
    ax.set_axis_off()
    panel_title(ax, "B", "Candidate disposition across four gates")
    ...
    boxes = [
        ...
        (f"{int(both.sum())} robust", "Molecular\nstability", BLUE_PALE, BLUE),
        ("Biomarker", "Human\nexposure\nfeasibility", BLUE_PALE, BLUE),
        ("Novelty", "CRC literature\ncollision", ORANGE_LIGHT, "#9B6117"),
        ("Epidemiology", "Human CRC\ntestability", ORANGE_LIGHT, "#9B6117"),
        ("Lead axis", "MiNP/DINP\n→ MCOP", GREEN_LIGHT, GREEN),
    ]
    x_positions = [0.005, 0.172, 0.339, 0.506, 0.673, 0.840]
    for i, ((headline, detail, face, edge), x) in enumerate(zip(boxes, x_positions)):
        card = rounded_card(ax, (x, 0.34, 0.145, 0.42), "", facecolor=face, edgecolor=edge, linewidth=1.0)
        t1 = ax.text(x + 0.0725, 0.625, headline, transform=ax.transAxes, ha="center", va="center",
                     fontsize=5.0, fontweight="bold", color=edge, clip_on=False)
        t2 = ax.text(x + 0.0725, 0.455, detail, transform=ax.transAxes, ha="center", va="center",
                     fontsize=5.0, color=NEUTRAL_DARK, linespacing=1.08, clip_on=False)
        card._contained_text = [t1, t2]
        if i < len(boxes) - 1:
            arrow(ax, (x + 0.15, 0.55), (x_positions[i + 1] - 0.005, 0.55), color=NEUTRAL, scale=6)
    ax.text(0.5, 0.16,
            "Molecular rank alone did not determine advancement; downstream gates are actionability filters",
...
def draw_panel_c(ax, matrix):
    ax.set_axis_off()
    panel_title(ax, "C", "Representative candidate evidence matrix")
    rows = matrix.copy()
    cols = ["Molecular\nsignal", "Stable", "Human\nbiomarker", "Multi-\ncycle", "Novelty\nhits*", "Human CRC\ngate"]
    x0, y0, w, h = 0.20, 0.10, 0.78, 0.77
    ...
    for j, col in enumerate(cols):
        ...
        ax.text(x0 + (j + 0.5) * cw, y0 + (nrow + 0.5) * rh, col, transform=ax.transAxes,
                ha="center", va="center", fontsize=5.0, fontweight="bold", color=NEUTRAL_DARK)
    for i, row in rows.iloc[::-1].reset_index(drop=True).iterrows():
        ...
        values = [row["molecular_signal"], row["molecular_stable"], row["human_biomarker"],
                  row["nhanes_cycles"], str(row["pubmed_crc_hits"]), row["human_crc_gate"]]
    ...
    ax.text(0.20, 0.025, "*Exact Title/Abstract query audit (23 Aug 2026); not exhaustive.  †BBzP represented by urinary MBzP.  NE = not evaluated.",
            transform=ax.transAxes, ha="left", va="bottom", fontsize=5.0, color=NEUTRAL)
...
def write_metadata(out_dir: Path, source_dir: Path, source_files, top30, matrix):
    stem = "Figure2_candidate_triage_v2"
    manifest_rows = []
    for path in source_files + [
        source_dir / "figure2_top30_screen_v2.csv",
        source_dir / "figure2_triage_matrix_v2.csv",
        source_dir / "figure2_pubmed_collision_audit_v2.csv",
        source_dir / "figure2_biomarker_translation_v2.csv",
    ]:
    ...
    qa = """# Figure 2 QA\n\n- PASS: all 30 nominees are plotted and rank order is locked.\n- PASS: Panel A encodes CRC overlap by point size and chemical class by marker shape.\n- PASS: MiNP is explicitly identified as rank 24.\n- PASS: the four downstream actionability gates are named explicitly.\n- PASS: eight representative candidates are shown, including TBPH.\n- PASS: unavailable/not-evaluated evidence is not encoded as a negative result.\n- PASS: BBzP-to-MBzP proxy status is disclosed.\n- PASS: exact-query collision audit is labelled non-exhaustive; TBPH is marked not audited.\n- PASS: card text containment check completed before export.\n- PASS: minimum rendered text size is 5 pt.\n- PASS: PDF, SVG, and 300-dpi PNG exported.\n"""
...
def main():
    ...
    save_outputs(fig, out / "Figure2_candidate_triage_v2")
    plt.close(fig)
