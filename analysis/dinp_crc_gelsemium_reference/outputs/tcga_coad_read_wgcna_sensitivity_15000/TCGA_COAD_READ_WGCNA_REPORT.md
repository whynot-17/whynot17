...
A signed, bicor WGCNA was built from the top 15000 genes selected by full-transcriptome MAD after gene-symbol deduplication. The fresh 41-gene DINP–CRC set was overlaid after network construction; target genes were not used to select or force network genes.

- Samples: **380** primary tumors (288 COAD, 92 READ).
- Network genes: **15000**; fresh 41-gene targets present as network genes: **26/41**.
- Targets projected to module eigengenes only: **15/41**.
- Non-grey modules: **3**; target-enriched modules by module-level BH-FDR<0.05: **0**.
- Soft-threshold power: **7** (first power with signed R² >= 0.80 and mean connectivity > 1).
...
- Network: signed bicor; power=7; minModuleSize=30; mergeCutHeight=0.25; deepSplit=2.
