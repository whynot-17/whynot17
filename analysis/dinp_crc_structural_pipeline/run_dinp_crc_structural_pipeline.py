...
def http_get(url: str, *, params=None, timeout=60) -> requests.Response:
    last = None
...
def http_post(url: str, *, json_body=None, data=None, timeout=60) -> requests.Response:
    last = None
    for attempt in range(4):
        try:
...
def download_pdbs(chosen: Dict[str, str], out_dir: Path, audit: Audit) -> None:
    ...
    for gene, pdb_id in chosen.items():
        try:
            ...
            audit.add("PDB_DOWNLOAD", "UNAVAILABLE", f"{gene}/{pdb_id}: {exc}")
